...
public class MegaLoftTerrainCarve : MonoBehaviour
{
	...
	public void SaveHeights()
	{
		...
		if ( terrain )
		{
			...
			savedheights = new float[tdata.heightmapResolution * tdata.heightmapResolution];
			Convert(heights, savedheights, tdata.heightmapResolution, tdata.heightmapResolution);
#else
			float[,] heights = tdata.GetHeights(0, 0, tdata.heightmapResolution, tdata.heightmapResolution);
			savedheights = new float[tdata.heightmapResolution * tdata.heightmapResolution];
			Convert(heights, savedheights, tdata.heightmapResolution, tdata.heightmapResolution);
	...
	public void ResetHeights()
	{
		...
		if ( terrain )
		{
			...
			if ( savedheights != null )
			{
				...
				Convert(savedheights, heights, tdata.heightmapResolution, tdata.heightmapResolution);
	...
	public void ConformTerrain()
	{
		if ( tobj )
		{
			...
			if ( col && terrain )
			{
				tdata = terrain.terrainData;
				float[,] heights = tdata.GetHeights(0, 0, tdata.heightmapResolution, tdata.heightmapResolution);
				float[,] newheights = new float[tdata.heightmapResolution, tdata.heightmapResolution];

				bool[,] hits = new bool[tdata.heightmapResolution, tdata.heightmapResolution];
				...
				{
					p.z = ((float)y / (float)tdata.heightmapResolution) * tdata.size.z;
					...
					for ( int x = 0; x < tdata.heightmapResolution; x++ )
					{
						...
						p.y *= tdata.size.y;
						p.x = ((float)x / (float)tdata.heightmapResolution) * tdata.size.x;
				...
				if ( numpasses > 0 )
				{
					float[,] smoothed = SmoothTerrain(newheights, numpasses, tdata.heightmapResolution, tdata.heightmapResolution, hits);
